"""
SSL Pretraining Dataset — _final
================================

Identical to ``ssl_dataset.py`` but imports ``config_final`` instead of
``config``. The only config reference is ``cfg.DATA_ROOT`` inside the labeled-
training branch of ``build_pretrain_manifest``; the MT trainer calls it with
``train_datasets=None`` (AADC-only), so that branch is skipped. No label
handling anywhere — unlabeled clips + source site only.
"""
...
class SSLClipDataset(Dataset):
    ...
    Each ``__getitem__`` returns ``{"audio": (n_samples,) float32, "site": str}``.
    The training loop produces the two augmented views per clip.
    """

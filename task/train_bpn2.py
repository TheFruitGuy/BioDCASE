"""
BPN Ladder (Phase 9) - Rung 2 / 9c: Multi-Tap Gate
==================================================

Third rung of the WhaleVAD-BPN ladder (tracked in W&B as phase ``9c`` within
the ``phase9_bpn_reproduction`` group). It extends rung 1's minimal gate from a
single tap to all three depthwise taps (h0, h1, h2), each with its own
projection head, combined into one ROI (``R = 1``) that drives the gate.

Everything else matches rung 1: focal loss on gated probabilities, BiLSTM ROI
processor, gate initialised to pass-through, the BPN training recipe (LR 1e-3,
weight decay 0.01, batch 48, <=32 epochs), and per-epoch negative resampling.

What this rung answers
----------------------
Whether using all three intermediate taps (vs the last tap only in rung 1)
improves the gate - i.e. how many taps the boundary proposal actually needs.

Usage
-----
::

    python train_bpn2.py
    python train_bpn2.py --init-from runs/bpn0_<timestamp>/bpn0_best.pt
    python train_bpn2.py --wandb
"""
...
def build_model(device: torch.device, init_from: str | None = None):
    """Multi-tap BPN model (R=1) + spectrogram extractor."""
    model = WhaleVADBPN(
        num_classes=7, use_bpn=True,
        bpn_taps=(0, 1, 2), bpn_R=1, bpn_use_bilstm=True,
    ).to(device)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="BPN ladder rung 2: multi-tap gate (R=1).")
...
def main():
    ...
    run_dir = Path(cfg.OUTPUT_DIR) / f"bpn2_{timestamp}"
    run_dir.mkdir(parents=True, exist_ok=True)
    print(f"Run dir: {run_dir}")

    print("\nConfiguration (BPN rung 2 - multi-tap gate):")
    print(f"  Taps: (0, 1, 2)   R: 1   ROI processor: BiLSTM")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save(ckpt, run_dir / f"bpn2_epoch_{epoch:02d}.pt")
        if improved:
            torch.save(ckpt, run_dir / "bpn2_best.pt")
    ...
    if run is not None:
        ...
        wbu.finalize_phase(
            history,
            verdict=(f"Multi-tap BPN gate (R=1): best micro {max(f1s):.3f}, "
                     f"best macro {max(macros):.3f}."),
            best_ckpt=run_dir / "bpn2_best.pt")

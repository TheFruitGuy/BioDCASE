...
def validate(model, spec_extractor, loader, criterion, device,
    ...
    # Macro F1 = mean of per-class F1s. This is the BioDCASE 2025 ranking
    # metric (Section 3.4 of the challenge description). Until now this
    # function returned ``overall_f1`` (i.e. micro F1) as ``mean_f1``,
    # which made LR scheduling, early-stop, and best-checkpoint
    # selection all optimise micro instead of macro. The wandb panel
    # also logged this value under the key ``val/f1_macro``, which was
    # silently wrong. Computing macro F1 explicitly here and returning
    # it as the selection metric fixes both.
    class_f1s = [metrics.get(name, {}).get("f1", 0.0) for name in cfg.CALL_TYPES_3]
    ...
    return {
        ...
        # ``mean_f1`` is the metric the main loop uses for LR scheduling,
        # best-checkpoint selection, and early-stop. It now points at the
        # macro F1, matching the BioDCASE 2025 ranking metric. The micro
        # F1 (formerly ``mean_f1``) is still available as ``overall_f1``
...
def main():
    ...
    for epoch in range(1, cfg.EPOCHS + 1):
        ...
        print(f"  Macro F1: {val['mean_f1']:.3f}  "
        ...
        wandb_payload = {
            ...
            "val/loss":      val["loss"],
            # val/f1_macro is the BioDCASE ranking metric (mean of
            # per-class F1s) and the value used for LR scheduling,
            # best-checkpoint, and early-stop. val/f1_micro is the
            # overall event-level F1, kept for backward comparison
            # with the old logs (which mislabelled this number as
            # "f1_macro").
            "val/f1_macro":  val["mean_f1"],
            "val/f1_micro":  val["overall_f1"],
        }

"""
Mean Teacher Self-Training — Core Components (_final)
=====================================================

Verbatim copy of ``mean_teacher_core.py``. The ONLY change is that the two
view builders lazy-import ``ssl_augmentations_final`` (which sources its
spectrogram constants from ``config_final``) instead of ``ssl_augmentations``,
so the whole MT stack is _final-native. The module itself imports no config
and is model-agnostic (spec extractor is passed in), so there is no USE_3CLASS
hazard.
...
# ======================================================================
# These are thin wrappers around ssl_augmentations_final.make_view that pin
...
def make_weak_view(audio, sites, spec_extractor):
    ...
    # Lazy import keeps this module decoupled from ssl_augmentations_final
    # for unit tests that don't need them.
    from ssl_augmentations_final import make_view
    return make_view(

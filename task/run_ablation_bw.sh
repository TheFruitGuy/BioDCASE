...
#   PY=...        python to use (default: the nave-bw conda env interpreter)
#   WORKERS=N     threshold-tuner workers (default 20)
#   EVAL_SEG=S    eval tile length in seconds (default 60)

set -u

# --- env hardening: make this launcher independent of the active conda env ---
# Calling an env's python by full path skips the per-env variables set via
# `conda env config vars`. Exporting them here keeps the ladder correct no
# matter how it is launched: PYTHONNOUSERSITE blocks the ~/.local NCCL shadow,
# CUDA_DEVICE_ORDER pins CUDA indices to nvidia-smi PCI order (so
# CUDA_VISIBLE_DEVICES=2 / =7 are the Blackwells).
export PYTHONNOUSERSITE=1
export CUDA_DEVICE_ORDER=PCI_BUS_ID

SEED="${1:?usage: CUDA_VISIBLE_DEVICES=<gpu> bash run_ablation.sh <seed>}"
PY="${PY:-/home/matthias-nagl/.conda/envs/nave-bw/bin/python}"
WORKERS="${WORKERS:-20}"
...
echo "    python : $PY"
$PY -c "import torch; print(f'    torch  : {torch.__version__}  archs={torch.cuda.get_arch_list()}')" 2>/dev/null \
  || echo "    torch  : !! could not import torch with this interpreter"

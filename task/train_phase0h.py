...
def main():
    ...
    run = wbu.init_phase("0h", config={
        ...
        "segment_s": PHASE0E_SEGMENT_S, "epochs": PHASE0F_EPOCHS,
    })
    ...
    # Stamp the computed weights onto the wandb run so they're visible
    # in the run's config alongside lr / batch_size / etc.
    run.config.update({
    ...
    # ------------------------------------------------------------------
    verdict_text = (
        f"Phase 0h: best F1 {max(f1s):.3f} at epoch "

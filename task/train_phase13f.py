"""
Phase 13f — consolidated strong base: FDY + dual-res + FilterAugment + EMA
==========================================================================
...
with the proven, structure-safe training recipe:
  ...
  * FDY stem (13d) + dual-resolution input  — the frequency-structure axis that
    gates the number (D's FM downsweep / precision).
...
This is just the 13d FDY trainer with `--dual-res --ema --filteraug` defaulted on,
logged under its own phase label. A bare ::

    CUDA_VISIBLE_DEVICES=<gpu> python train_phase13f.py

runs the full recipe. For the attribution ablations, run the 13d trainer with the
flags off/subset (e.g. `train_phase13d.py --dual-res --ema` to drop FilterAugment),
which logs under 13d so the pair is easy to compare. Multi-seed the winner.

The saved checkpoint is evaluated with `eval_conformer.py` exactly like 13d — it
auto-detects the FDY stem and the 4-channel input from arch_kwargs; EMA and
FilterAugment are training-only and need no eval-side handling.
"""
...
def main():
    ...
    # The 13f recipe: near-base optimiser + the full frontend/augment/EMA stack on.
    p.set_defaults(peak_lr=5e-5, warmup_epochs=0.0, lr_schedule="const",
                   dual_res=True, ema=True, filteraug=True)
    a = p.parse_args()

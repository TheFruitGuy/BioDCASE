"""
Phase 6 (7-class variant): HNM Fine-Tune for 7-Class Models with PGI
======================================================================

Same HNM/PGI logic as ``train_phase_hnm.py``, but adapted for
checkpoints whose classifier outputs the 7 fine-grained call types
(bma, bmb, bmz, bmd, bpd, bp20, bp20plus). Run this when
``cfg.USE_3CLASS=False``; the 3-class version of the script bakes in
``cfg.CALL_TYPES_3.index(target_class)`` for PGI, which silently
degrades to "isolate the first 3 of 7 channels" on a 7-class model.

What changes relative to the 3-class script
-------------------------------------------

The ONLY substantive change is ``build_hard_neg_class_map``. It now
expands ``target_class`` to the appropriate set of 7-class channel
indices depending on the JSON's labelling:

  - Group-level PGI (target_class in CALL_TYPES_3, e.g. "d"):
    isolate the GROUP of 7-class channels that collapses to that
    3-class target under COLLAPSE_MAP. For "d" the mask retains
    {bmd, bpd} and zeros the other five channels.

  - Subclass-level PGI (target_class in CALL_TYPES_7, e.g. "bmd"):
    isolate that single 7-class channel. Strictest cross-class
    isolation; only makes sense if the mining was done in 7-class
    space against subclass GT.

Either kind of JSON works. You can mix them in one call (pass several
``--hard_negatives``). Both come out of ``mine_hard_negatives_7class.py``;
group-level JSONs from the 3-class miner ARE NOT compatible because
that miner is broken for 7-class models — see that script's header.

Validation/eval is unchanged: probabilities are collapsed 7→3 before
F1 is computed, so the macro/overall numbers remain comparable to
3-class HNM runs.
...
between 3-class and 7-class HNM runs.

Usage
-----
::

    # Group-level PGI with the existing D + BMABZ mining workflow:
    python train_phase_hnm_7class.py \\
        --checkpoint runs/phase0r_20260520_xxxxxx/best_model.pt \\
        --hard_negatives runs/hardnegs_7c/d_<src>.json \\
                         runs/hardnegs_7c/bmabz_<src>.json \\
        --isolate_classes \\
        --epochs 15 --lr 1e-5 --oversample 5

    # Subclass-level PGI (strictest test of the PGI hypothesis):
    python train_phase_hnm_7class.py \\
        --checkpoint runs/phase0r_20260520_xxxxxx/best_model.pt \\
        --hard_negatives runs/hardnegs_7c/bmd_<src>.json \\
                         runs/hardnegs_7c/bpd_<src>.json \\
        --isolate_classes \\
        --epochs 15 --lr 1e-5 --oversample 5

    # Ablation C (no PGI): omit --isolate_classes from either above.
"""
...
def _group_to_7c_indices() -> dict[str, set[int]]:
    """3-class group name -> set of 7-class indices that collapse to it.

    Built from cfg.COLLAPSE_MAP, so any future regrouping there
    propagates here without code changes. With the standard map this
    yields:
        bmabz -> {0, 1, 2}    # bma, bmb, bmz
        d     -> {3, 4}       # bmd, bpd
        bp    -> {5, 6}       # bp20, bp20plus
    """
    return {
        c3: {i for i, c7 in enumerate(cfg.CALL_TYPES_7)
...
def build_hard_neg_class_map(hard_segs, used_records):
    """Map (dataset, filename, start_sample) -> set of 7-class indices.

    The PGI mask in 7-class mode supports two granularities, chosen
    per-record by the value of ``target_class`` in the mining JSON:

      - ``target_class`` in CALL_TYPES_3 (e.g. "d"): GROUP PGI. The
        mask retains the set of 7-class channels that collapse to this
        3-class group under COLLAPSE_MAP. Reuses the existing 3-class
        mining workflow; the 7-class advantage materialises purely
        through finer gradient isolation at FP locations (other
        groups' channels stay clean).

      - ``target_class`` in CALL_TYPES_7 (e.g. "bmd"): SUBCLASS PGI.
        The mask retains only that single channel. The strictest form
        of cross-class isolation. Requires 7-class mining (see
        ``mine_hard_negatives_7class.py``).

    Records with different granularities can be mixed in one run.
    """
    if len(hard_segs) != len(used_records):
        raise ValueError(
            f"Length mismatch: {len(hard_segs)} segments vs "
            f"{len(used_records)} fp records.")
    group_map = _group_to_7c_indices()
    ...
    for seg, rec in zip(hard_segs, used_records):
        ...
        if tc in cfg.CALL_TYPES_7:
            idxs = {cfg.CALL_TYPES_7.index(tc)}            # subclass PGI
        elif tc in cfg.CALL_TYPES_3:
            idxs = set(group_map[tc])                       # group PGI
        else:
            raise ValueError(
                f"Unknown target_class {tc!r}; expected one of "
                f"{cfg.CALL_TYPES_7} or {cfg.CALL_TYPES_3}.")
...
def main():
    ...
    # processes are unaffected. Must happen before any model / dataset
    # / dataloader construction because those read n_classes() at
    # build time.
    if getattr(cfg, "USE_4CLASS_D_SPLIT", False):
        raise RuntimeError(
            "USE_4CLASS_D_SPLIT must be False for 7-class HNM; "
            "got True. Check config.py.")
    if cfg.USE_3CLASS:
        print(f"[hnm 7c] cfg.USE_3CLASS True -> False (in-process "
    ...
    extra_tags.append("7class")  # distinguishes from the 3-class HNM runs
    ...
    if args.isolate_classes:
        # Distinguish group vs subclass PGI for the runs table.
        n_subclass = sum(1 for rec in used_records
                         if rec["target_class"] in cfg.CALL_TYPES_7)
        n_group = sum(1 for rec in used_records
                      if rec["target_class"] in cfg.CALL_TYPES_3)
        multi = sum(1 for v in hard_neg_class_map.values() if len(v) > 1)
        pgi_mode = ("subclass" if n_subclass and not n_group else
                    "group" if n_group and not n_subclass else
                    "mixed")
        print(f"  PGI on ({pgi_mode}): {len(hard_neg_class_map)} unique "
              f"hard-neg locations ({multi} active on >1 channel)")
        print(f"    subclass-target records: {n_subclass}")
        print(f"    group-target records:    {n_group}")
        run.config.update({"n_pgi_locations": len(hard_neg_class_map),
                           "n_pgi_multi_class": multi,
                           "pgi_mode": pgi_mode,
                           "n_subclass_target_records": n_subclass,
                           "n_group_target_records": n_group},
                          allow_val_change=True)
    else:
        print(f"  PGI off: standard multi-class loss on all 7 channels")
    ...
    # ------------------------------------------------------------------
    import wandb
    # When subclass-target JSONs (e.g. target_class="bmd") are loaded,
    # roll them up to their 3-class group via COLLAPSE_MAP so the
    # is_target_<3c> flags remain queryable in the runs table alongside
    # 3-class HNM runs.
    targets_used_3c = set()
    for t in targets_used:
        if t in cfg.CALL_TYPES_3:
            targets_used_3c.add(t)
        elif t in cfg.CALL_TYPES_7:
            targets_used_3c.add(cfg.COLLAPSE_MAP[t])
    for t in cfg.CALL_TYPES_3:
        ...
        wandb.summary[f"is_target_{t}"] = (t in targets_used_3c)

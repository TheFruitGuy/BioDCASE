"""
Phase 8b: Per-Class Focal-α Fine-Tune (Phase 8 v2)
====================================================

Replaces Phase 8a's per-class γ approach with a per-class α approach,
after empirical evidence that γ_d=4 with α=0.25 actually decreased D's
effective loss weighting in 4/4 runs.

What went wrong in v1
---------------------
Phase 8a raised γ_d from 2.0 to 4.0 with α=0.25 across all classes.
The intent was "sharper hard-positive emphasis on D". The actual effect:

  Effective D positive loss per element (relative to v1's scalar focal):
    pos_weight_d * α_d * (1 - p_t)^γ_d * log(p)
    = 3.557 * 0.25 * (1 - p_t)^4 * log(p)

  At p_t=0.5 a γ=4 modulation gives 0.0625, vs γ=2's 0.25 -- a 4x
  *reduction*. Raising γ shrinks loss magnitude across the board; the
  classic fix is to raise α correspondingly. We did not. So D's positive
  signal got quieter relative to bp (pos_weight 12.6, γ=2), not louder.

Empirically: 3/4 v1 runs went backwards on macro F1, the 4th moved
+0.003 (within the v1 smoke test noise band). D recall oscillated 0.09 -
0.24 across consecutive epochs with no clear trend. The technique as
implemented in v1 is a documented negative result.

What v2 does differently
------------------------
* **γ_d back to 2.0** (standard) -- removes the magnitude-reduction
  side effect. We are no longer trying to sharpen the focusing curve,
  only to reweight the positive/negative balance per class.
* **α_d = 0.6** (default, up from 0.25) -- this is the actual lever
  for boosting D's positive contribution to the loss. At p_t=0.5:
    v1 (α=0.25, γ=4): 0.0625 * 0.25 = 0.0156
    v2 (α=0.6,  γ=2): 0.25 * 0.6   = 0.150
  ~10x more D positive signal per element than v1, ~2.4x more than the
  original training (α=0.25, γ=2 -> 0.0625).
* **lr = 1e-5** (HNM's stable rate, up from v1's overly-conservative
  5e-6). The v1 smoke test oscillated at lr=5e-6 even with the
  no-op γ=2/α=0.25 config; HNM was stable at 1e-5 with a more
  invasive intervention. Higher LR may actually reduce the
  fine-tune-form oscillation by giving the model enough push to leave
  the immediate local-minimum basin coherently.

Everything else (architecture auto-detect, dataset, validation,
threshold tuning) is identical to v1. The per-class focal-loss code is
unchanged -- it already supports per-class α via the same tensor path
v1 used for γ. We are only changing which knob we turn.

Source-checkpoint choice
------------------------
Same as v1: HNM_D checkpoints to stack v2 on top of HNM, or pre-HNM
checkpoints for a clean comparison. Given v1's negative result we
recommend starting with a single HNM_D checkpoint to check whether
α-tuned per-class focal moves the needle at all before sweeping.

Smoke-test recipe
-----------------
::

    # Identity smoke test: α=0.25 across all classes, γ=2 across all.
    # This IS standard focal -- if it oscillates at lr=1e-5, the
    # technique is dead and we should not run the real version. If it
    # holds within ±0.005 of source macro F1, lr=1e-5 is stable for
    # fine-tune-form per-class focal.
    python train_phase8_v2.py \\
        --checkpoint runs/hnm_D_phase5_20260507_211504/best_model.pt \\
        --alpha_d 0.25 --gamma_d 2.0 \\
        --epochs 5 --run_name phase8b_smoke

Real-experiment recipe
----------------------
::

    # Single-checkpoint test of v2 hypothesis: α_d=0.6, γ_d=2, lr=1e-5:
    python train_phase8_v2.py \\
        --checkpoint runs/hnm_D_phase5_20260507_211504/best_model.pt

    # If above moves the needle, sweep on the remaining three:
    for CKPT in \\
        runs/hnm_D_whalevad_20260504_152450/best_model.pt \\
        runs/hnm_D_phase5_20260506_204358/best_model.pt \\
        runs/hnm_D_whalevad_20260507_191223/best_model.pt; do
        python train_phase8_v2.py --checkpoint $CKPT
    done

    # Sweep α_d to find the optimum if v2 worked:
    for AD in 0.4 0.6 0.75; do
        python train_phase8_v2.py \\
            --checkpoint runs/hnm_D_phase5_20260507_211504/best_model.pt \\
            --alpha_d $AD --run_name phase8b_ad${AD}_sweep
    done
...
Add this entry to ``PHASE_REGISTRY`` in ``wandb_utils.py``::
    ...
    "8b": dict(
        parent="8",
        hypothesis=("Per-class focal α fine-tune (Phase 8 v2). v1 raised "
                    "γ_d to 4.0 with α=0.25 and the focal-magnitude side "
                    "effect actually decreased D's effective loss weight, "
                    "yielding 3/4 negative runs. v2 holds γ=2 across "
                    "classes (standard) and raises α_d to 0.6 to directly "
                    "boost D's positive contribution. lr=1e-5 (HNM's "
                    "stable rate, vs v1's 5e-6 which oscillated even on "
                    "no-op smoke test)."),
        interventions=["per_class_focal_alpha"],
    ),

If the "8" entry exists (from v1), this just adds the child. Without
either entry, ``wbu.init_phase("8b", ...)`` will raise.
"""
...
# Default focal hyperparameters for v2. γ stays uniform at 2.0 (standard),
# and α_d is the lever. α_bmabz and α_bp stay at 0.25 (standard).
PHASE8_DEFAULT_GAMMA = {"bmabz": 2.0, "d": 2.0, "bp": 2.0}
PHASE8_DEFAULT_ALPHA_D = 0.6
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--gamma_d", type=float,
                   ...
                   help=f"Focal γ for the D class. v2 default: "
                        f"{PHASE8_DEFAULT_GAMMA['d']} (back to standard, "
                        f"matching the other classes). v1 used γ_d=4 and "
                        f"that backfired -- focal magnitude scales down "
                        f"with γ, which weakened D's signal rather than "
                        f"strengthening it. Leave at 2.0 unless you're "
                        f"explicitly running a γ-sweep ablation.")
    ...
    p.add_argument("--alpha", type=float, default=PHASE8_DEFAULT_ALPHA,
                   help="Scalar α default for classes without an explicit "
                        "per-class value (default 0.25). The v2 hypothesis "
                        "is that per-class α (not γ) is the right lever; "
                        "see --alpha_d in particular.")
    p.add_argument("--alpha_bmabz", type=float, default=None,
                   help="Per-class α for bmabz. Default: --alpha (0.25).")
    p.add_argument("--alpha_d", type=float, default=PHASE8_DEFAULT_ALPHA_D,
                   help=f"Per-class α for D. **The main v2 lever.** "
                        f"Default: {PHASE8_DEFAULT_ALPHA_D}. The original "
                        f"training used 0.25; v2 raises this to boost D's "
                        f"positive contribution to the loss without "
                        f"changing the focal-magnitude curve. Worth "
                        f"sweeping if the default works: try 0.4, 0.6, 0.75.")
    p.add_argument("--alpha_bp", type=float, default=None,
                   help="Per-class α for bp. Default: --alpha (0.25).")

    # Training schedule.
    p.add_argument("--epochs", type=int, default=PHASE8_DEFAULT_EPOCHS)
    p.add_argument("--lr", type=float, default=PHASE8_DEFAULT_LR,
                   help="Fine-tune LR. v2 default 1e-5 matches HNM's "
                        "stable rate; v1's 5e-6 oscillated even on the "
                        "no-op smoke test, so we suspect over-conservative "
                        "LR contributed to v1's instability.")
...
def main() -> None:
    ...
    print(f"\nPer-class focal config (Phase 8 v2):")
    for i, name in enumerate(cfg.CALL_TYPES_3):
        print(f"  {name:<6} γ={gamma[i].item():.2f}  α={alpha[i].item():.3f}")

    # v2 sanity check: γ_d != γ_others is the v1 anti-pattern.
    if args.gamma_d != args.gamma_bmabz or args.gamma_d != args.gamma_bp:
        print(f"  NOTE: γ_d differs from γ_bmabz/γ_bp. v2's hypothesis is "
              f"that α (not γ) is the right lever — if you're intentionally "
              f"doing a γ-ablation, fine; otherwise consider holding γ "
              f"uniform.")
    if (gamma == gamma[0]).all() and (alpha == alpha[0]).all():
        print(f"  WARNING: γ and α are identical across classes; this is "
              f"the no-op smoke-test config. Useful only for checking "
              f"fine-tune stability at lr={args.lr}.")
    ...
    # Wandb init.
    # ------------------------------------------------------------------
    extra_tags = [f"ad_{a_d:.2f}", f"abmabz_{a_bmabz:.2f}",
                  f"abp_{a_bp:.2f}",
                  f"gd_{args.gamma_d:g}", f"gbmabz_{args.gamma_bmabz:g}",
    ...
    run = wbu.init_phase("8b", config=config_payload,
                         name_suffix=source_short, extra_tags=extra_tags)

    run_name = (args.run_name
                or (f"phase8b_ad{a_d:g}_{source_short}"
    ...
    print(f"\n{'=' * 60}")
    print(f"Phase 8 v2 fine-tune {args.epochs} epochs @ lr={args.lr}, "
          f"select_by={args.select_by}")
    print(f"  starting {args.select_by} F1: {best_f1:.3f}")
    print(f"  γ = {gamma.tolist()}  α = {alpha.tolist()}")
    print(f"  v2 lever: α_d={alpha[cfg.CALL_TYPES_3.index('d')].item():.2f} "
          f"(vs original training's 0.25)")
    print(f"{'=' * 60}")
    ...
    wandb.summary["alpha_d"] = float(a_d)
    wandb.summary["alpha_bmabz"] = float(a_bmabz)
    wandb.summary["alpha_bp"] = float(a_bp)
    ...
    if selected_delta > 0.005:
        verdict_text = (
            f"Phase 8 v2 helped: {args.select_by} F1 "
            f"{val0[f'{args.select_by}_f1']:.3f} → {best_f1:.3f} "
            f"({selected_delta:+.3f}). D recall {d_recall_delta:+.3f} "
            f"with α_d={a_d}. Source phase {source_phase}, HNM stacked={use_hnm}. "
            f"v1's γ-based approach did not work; v2's α-based one did."
        )
    elif selected_delta > -0.005:
        verdict_text = (
            f"Phase 8 v2 neutral: {args.select_by} F1 "
            f"{val0[f'{args.select_by}_f1']:.3f} → {best_f1:.3f} "
            f"({selected_delta:+.3f}). D recall {d_recall_delta:+.3f}. "
            f"α_d={a_d} did not move the needle past noise. Try a "
            f"different α_d (currently {a_d}), or consider that per-class "
            f"focal in fine-tune form may not be effective regardless of "
            f"hyperparameters (v1's γ also failed)."
        )
    else:
        verdict_text = (
            f"Phase 8 v2 hurt: {args.select_by} F1 "
            f"{val0[f'{args.select_by}_f1']:.3f} → {best_f1:.3f} "
            f"({selected_delta:+.3f}). α_d={a_d} may be too aggressive "
            f"(overshoots positive emphasis and tanks precision), or "
            f"lr={args.lr} too high for this source. Phase 8 v1 also "
            f"failed; combined evidence suggests fine-tune-form per-class "
            f"focal is unlikely to work."
        )
